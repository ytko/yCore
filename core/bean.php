...
class yBean extends yBase{
	...
	public function get () {
		$controller = yCore::getController($this->controller);

		$model = yCore::getModel($this->modelName)
				->setController($controller);

		$view = yCore::getView($this->viewName);
		$template = yCore::getTemplate($this->templateName)
				->setModel($model);
